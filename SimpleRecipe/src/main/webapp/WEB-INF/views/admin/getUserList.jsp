...
<div class="paging-wrap">
	<ul> 
		<c:if test="${pageMaker.prev}">
			<li><a href="getUserList.do${pageMaker.makeQuery(pageMaker.startPage - 1)}"><</a></li>
		</c:if>
		<c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
			<li><a href="getUserList.do${pageMaker.makeQuery(idx)}">${idx}</a></li>
		</c:forEach>
		<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
			<li><a href="getUserList.do${pageMaker.makeQuery(pageMaker.endPage + 1)}">></a></li>
		</c:if>
	</ul>
</div>
</body>
